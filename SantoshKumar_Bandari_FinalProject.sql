...
--Creating user VPD_CLERK1

Drop User VPD_CLERK1 Cascade;
Create User VPD_CLERK1 Identified BY "jessie$22"
Default TABLESPACE USERS
Temporary TABLESPACE TEMP;
GRANT CONNECT TO VPD_CLERK1;


--Creating user VPD_CLERK2
...
Grant connect,DBA,create session to DBSEC with admin option;
...
-------3-creating complexity function,roles,profiles
...
(username varchar2,
  ...
  RETURN boolean IS 
   ...
   pchar  boolean;
   punct boolean;
   num_array varchar2(20);
   symbol_array varchar2(25);
   alpha_array varchar2(52);

BEGIN 
   num_array:= '0123456789';
   alpha_array:= 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ';
   symbol_array:='!"#$%&()``*+,-/:;<=>?_';

  -- validation for password differs username or not
   IF NLS_LOWER(password) = NLS_LOWER(username) THEN
     raise_application_error(-20101, 'Password should not be similar to username');
   END IF;

   -- validation for length of the password is 6 or not
   IF length(password) < 6 THEN
      raise_application_error(-20102, 'Password length must not be less than 6');
   END IF;

   --validation for password is simple or obvious, such as welcome, account , oracle, database , or user
   IF NLS_LOWER(password) IN ('welcome', 'database', 'account', 'user', 'oracle') THEN
      raise_application_error(-20103, 'Password cannot be welcome, account , oracle, database , or user ');
   END IF;

   -- validation to check if the password contains at least one letter, one digit and one punctuation mark.
   -- Check for number
   pdigit:=FALSE;
   y := length(password);
   FOR i IN 1..10 LOOP 
      FOR j IN 1..y LOOP 
         IF substr(password,j,1) = substr(num_array,i,1) THEN
            pdigit:=TRUE;
             GOTO funt_char;
         END IF;
      END LOOP;
   END LOOP;
   IF pdigit = FALSE THEN
      raise_application_error(-20104, 'Password should contain at least one digit');
   END IF;

   -- Check for the character
   <<funt_char>>
   pchar:=FALSE;
   FOR i IN 1..length(alpha_array) LOOP 
      FOR j IN 1..y LOOP 
         IF substr(password,j,1) = substr(alpha_array,i,1) THEN
            pchar:=TRUE;
             GOTO funt_sybmol;
         END IF;
      END LOOP;
   END LOOP;
   IF pchar = FALSE THEN
      raise_application_error(-20105, 'Password should contain at least one character');
   END IF;
   ...
   <<funt_sybmol>>
   punct:=FALSE;
   FOR i IN 1..length(symbol_array) LOOP 
      FOR j IN 1..y LOOP 
         IF substr(password,j,1) = substr(symbol_array,i,1) THEN
            punct:=TRUE;
             GOTO previous_search;
         END IF;
      END LOOP;
   END LOOP;
   IF punct = FALSE THEN
      raise_application_error(-20106, 'Password should contain at least one punctuation');
   END IF;

   <<previous_search>>
   -- validation to check if the password is same as the old password for alteast 3 characters 
   IF old_password IS NOT NULL THEN
     ...
     IF abs(diff) < 3 THEN
       ...
       IF diff < 3 THEN
         raise_application_error(-20107, 'Password should differ by at least 3 characters');
       END IF;
     END IF;
   END IF;
...
Grant Select, Alter On CUSTOMER  To DBSEC_ADMIN_ROLE;

DROP ROLE DBSEC_CLERK_ROLE Cascade;
Create ROLE DBSEC_CLERK_ROLE;
Grant Select, Insert, Update On CUSTOMER To DBSEC_CLERK_ROLE;

Drop ROLE DBSEC_SUPERVISOR_ROLE Cascade;
CREATE ROLE DBSEC_SUPERVISOR_ROLE;
Grant Select, Insert, Update, Delete On CUSTOMER To DBSEC_SUPERVISOR_ROLE;

Drop ROLE DBSEC_QUERY_ROLE Cascade;
Create ROLE DBSEC_QUERY_ROLE;
Grant Select On CUSTOMER To DBSEC_QUERY_ROLE;
...
Select SALES_REP_ID, CUSTOMER_ID, CUSTOMER_SSN, FIRST_NAME, LAST_NAME, ADDR_LINE,
CITY, STATE, ZIP_CODE, PHONE, EMAIL, CC_NUMBER, CREDIT_LIMIT, GENDER, STATUS, COMMENTS,
CTL_UPD_DTTM, CTL_UPD_USER, CTL_REC_STAT From Customer Where CTL_UPD_USER = USER;


GRANT SELECT,INSERT,UPDATE ON CUSTOMER_F_VIEW TO VPD_CLERK1;

CONNECT VPD_CLERK1/jessie$22;

select count(SALES_REP_ID) from DBSEC.CUSTOMER_F_VIEW where SALES_REP_ID =6415;

---6-implementing virtual private db
...
RETURN 'CTL_UPD_USER=USER';
end if;
end;
/


EXEC DBMS_RLS.DROP_POLICY('DBSEC','CUSTOMER','MY_OWNER_POLICY');

BEGIN
DBMS_RLS.ADD_POLICY( OBJECT_SCHEMA =>'DBSEC',
...
END;
/
select CTL_UPD_USER, COUNT(*) FROM DBSEC.CUSTOMER group by CTL_UPD_USER;

-------7-Audit the activities on CUSTOMER
...
------8-Audit the user activities
select OS_USERNAME, USERNAME, TIMESTAMP, ACTION_NAME From DBA_AUDIT_TRAIL WHERE USERNAME='VPD_CLERK1';
Select OS_USERNAME, USERNAME, TIMESTAMP, ACTION_NAME From DBA_AUDIT_TRAIL WHERE USERNAME='VPD_CLERK2';
